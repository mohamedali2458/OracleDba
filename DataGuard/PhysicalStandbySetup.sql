...
The standby database has Oracle 19c installation done in the same oracle home 
location as primary.
...
SQL> select GROUP#, THREAD#, bytes/1024/1024 size_mb, MEMBERS, STATUS from v$log order by 1;
...
ALTER DATABASE ADD STANDBY LOGFILE THREAD 1 GROUP 11 '/u01/app/oracle/oradata/ORADB/stb_redo1.log' SIZE 200M;
ALTER DATABASE ADD STANDBY LOGFILE THREAD 1 GROUP 12 '/u01/app/oracle/oradata/ORADB/stb_redo2.log' SIZE 200M;
ALTER DATABASE ADD STANDBY LOGFILE THREAD 1 GROUP 13 '/u01/app/oracle/oradata/ORADB/stb_redo3.log' SIZE 200M;
ALTER DATABASE ADD STANDBY LOGFILE THREAD 1 GROUP 14 '/u01/app/oracle/oradata/ORADB/stb_redo4.log' SIZE 200M;
...
SID_LIST_LISTENER =
  (SID_LIST =
   (SID_DESC=
     (GLOBAL_DBNAME=oradb_s2.localdomain)
     (SID_NAME=oradb_s2)
     (ORACLE_HOME=/u01/app/oracle/product/19.0.0/db_1)
   )
   (SID_DESC =
      (GLOBAL_DBNAME = oradb_s2_DGMGRL)
      (ORACLE_HOME = /u01/app/oracle/product/19.0.0/db_1)
      (SID_NAME = oradb_s2)
   )
  )
...
On Primary Server
=================
SQL> alter system set log_archive_dest_2 = 'service=oradb_s2 async valid_for=(online_logfiles,primary_role) db_unique_name=oradb_s2';

Set FAL_SERVER: Fetch Archive Log parameter tells primary as to where it will get archives from

On Primary Server
=================
SQL> alter system set fal_server = 'oradb_s2';

Set dg_config parameter: This parameter defines which databases are in data guard configuration

On Primary Server
=================
SQL> alter system set log_archive_config = 'dg_config=(oradb,oradb_s2)';
...
Bounce database & start MRP (Apply Service): Its good to bounce standby, put it in mount mode 
