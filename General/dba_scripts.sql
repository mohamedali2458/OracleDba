...
All the DBA Scheduler jobs create logs. You can query below and check the details of job logs
...
The below Linux command will help you to find files older than 35 days in a
specific directory path and save the ouput in backupfiles.log

Here the directory we are searching is /backup/logs and -mtime specifies
the modified time of a file. We are saving the list of all the files which are
...
If you want to print files older than 7 days on screen and do not want to
...
Below linux command will find all the files under current location (as we
have specified . dot), search file name starting with arch and ending with
...
How to analyze wait events in Oracle
====================================
Use below query to get the top wait classes in Oracle database
...
Use below command to enable Linux GUI interface at command prompt as root user:
...
Now you can invoke any Linux GUI interface by directly running the installer!
...
The error is because of permission issues on /usr/bin/crontab file. Login as
...
How to check FRA location utilization in Oracle
===============================================
Flash Recovery Area must be monitored regularly. Sometimes FRA runs out
...
last modified table in oracle. The table modification can be insert, update
or delete. Below queries get details of last or latest modified table in oracle
...
below query to find the archive space requirements and you can use it to
...
How to disable firewall in Linux 7
==================================
Disabling firewall in Linux 5/7 versions is little bit different than Linux 7.
Sometimes you need to disable firewall in Linux 7 version as part of
Database installation pre-requisites. This article will help you to find the
...
You can enable/disable 􀁓rewall completely on Linux with below commands:
...
How to check database lock conflict in Oracle
=============================================
Database lock conflicts are one of the issues which DBA needs to deal
with. The database locks can keep users waiting for very long and we must
...
How to configure yum server in linux
====================================
In this article, we will learn how to configure yum server in different Oracle
...
internet connection, you can configure YUM repository on Linux and this
...
Download and copy the appropriate yum configuration file in place, by
...
Download and install Oracle Linux and make sure you are able to connect
...
How to check query plan change in oracle
========================================
If you would like to find out change in SQL plan of a query, below script will
help you find the SQL plan ID for previous executions and check if there is
...
How to force users change password on first login Linux
=======================================================
How to force users change their passwords upon first login in Linux? How
to make sure user changes password at next login time in Linux?

You can force a user to change their password upon first time login to
Linux server. You can even force existing users to change their passwords
on next login. This is done using chage command in Linux. The chage
...
How to check datafile utilization in Oracle
===========================================
When you want to shrink a datafile, you must always check the single
datafile utilization. In case if you shrink datafile more than the used size, it
will fail. Below query gives the datafile utilization and depending upon the
...
How to estimate flashback destination space
===========================================
Sometimes application team will ask DBA to enable flashback for x
number of days. In such case, a DBA needs to estimate the flashback
space required for x number of days in order to store the flashback logs.
