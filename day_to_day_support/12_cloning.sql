...
production db at the time of backup. We need to apply only those archives and only one 
current redo log file.

When we say until cancel system will wait for our input until we supply required archives 
and current online redo log file like below:
...
To do this type of cloning we need cold backup files of a database. We only need online 
redo log files and datafiles. Controlfiles will be created when we execute control.sql file.
export ORACLE_SID=prod
sqlplus / as sysdba
startup
