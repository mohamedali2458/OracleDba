...
Pfile - pfile is a static, client-side text file that must be updated 
with a standard text editor like "notepad" or "vi".

SPfile - As SPFILE (Server Parameter File), on the other hand, is a 
persistent server side binary file that can only be modified with the "ALTER SYSTEM" command.
...
The above parameter value gets changed only in memory, 
to permanently change do the following:-
...
v$parameter2
V$PARAMETER2 displays information about the initialization parameters that are currently 
in effect for the session, with each list parameter value appearing as a row in the view. 
A new session inherits parameter values from the instance-wide values displayed in 
the V$SYSTEM_PARAMETER2 view.
...
If a server parameter file was not used to start the instance, then each row of the 
view will contain FALSE in the ISSPECIFIED column.
