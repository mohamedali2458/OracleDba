...
=========================
By default each database will have five tablespaces from oracle 11g.
...
dba_tablespaces	: DBA_TABLESPACES describes all tablespaces in the database.
...
dba_data_files	: DBA_DATA_FILES describes database files.

dba_extents		: DBA_EXTENTS describes the extents comprising the segments in all tablespaces in the database.
Note: that if a datafile (or entire tablespace) is offline in a locally managed tablespace, 
you will not see any extent information. If an object has extents in an online file of the 
tablespace, you will see extent information about the offline datafile. However, if the 
object is entirely in the offline file, a query of this view will not return any records.

USER_EXTENTS 	: describes the extents comprising the segments owned by the current user's objects. 
				  This view does not display the OWNER, FILE_ID, BLOCK_ID, or RELATIVE_FNO columns.
...
user_tablespaces: USER_TABLESPACES describes the tablespaces accessible to the current user. 
				  Its columns (except for PLUGGED_IN) are the same as those in DBA_TABLESPACES.

dba_segments	: DBA_SEGMENTS describes the storage allocated for all segments in the database.

USER_SEGMENTS 	: describes the storage allocated for the segments owned by the current user's 
				  objects. This view does not display the OWNER, HEADER_FILE, HEADER_BLOCK, 
...
dba_temp_files	: DBA_TEMP_FILES describes all temporary files (tempfiles) in the database.

dba_tablespace_groups : DBA_TABLESPACE_GROUPS describes all tablespace groups in the database.
...
v$undostat		: V$UNDOSTAT displays a histogram of statistical data to show 
				  how well the system is working. The available statistics include 
				  undo space consumption, transaction concurrency, and length of 
				  queries executed in the instance. You can use this view to 
				  estimate the amount of undo space required for the current 
				  workload. Oracle uses this view to tune undo usage in the 
				  system. The view returns null values if the system is in 
				  manual undo management mode.
					
				  Each row in the view keeps statistics collected in the instance 
				  for a 10-minute interval. The rows are in descending order by 
				  the BEGIN_TIME column value. Each row belongs to the time 
				  interval marked by (BEGIN_TIME, END_TIME). Each column 
				  represents the data collected for the particular statistic 
				  in that time interval. The first row of the view contains 
				  statistics for the (partial) current time period. The view 
...
dba_undo_extents: DBA_UNDO_EXTENTS describes the extents comprising the segments 
				  in all undo tablespaces in the database.

View for Sysaux tablespace
==========================
v$sysaux_occupants	: V$SYSAUX_OCCUPANTS displays SYSAUX tablespace occupant information.
